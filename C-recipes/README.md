...
- [`stack-catalog.md`](stack-catalog.md) — from Ch. 6 (the stacks the war stories come from: Forge + leads-crm + magma-core on Rails; canvas-mcp + coide + presentation-studio-mcp + markdown-toolkit on Node/TypeScript; plus the operator's magmalabs-assistant config).
